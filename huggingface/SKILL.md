---
name: huggingface
description: Mandatory for operational Hugging Face Hub tasks involving model repos, model cards, benchmark tables, `hf` CLI, `hf download`, `hf download --dry-run`, `hf cache`, `HF_HOME`, local Hugging Face caches, multiple cache directories, model availability, incomplete or metadata-only downloads, cache cleanup, disk-space planning, quantized model selection, download command generation, and benchmark provenance across Hugging Face pages. Use when the user asks which HF models are downloaded or missing, how large repos are, what to download under a size limit, how to filter/compare Hugging Face model variants, how to reconcile metrics from model cards or comparison tables, or how to produce cache-aware `hf` commands. Do not use for generic ML theory, model fine-tuning/training, dataset labeling, inference server deployment, or Python package installation unrelated to Hugging Face cache/model operations.
license: MIT
